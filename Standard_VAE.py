...
if __name__ == '__main__':
    ...
    t = F.sigmoid(t)
    t = t.to(device)
    recon_t, mu, logvar = model(t)
    loss, BCE_LOSS, KLD_LOSS = loss_function(recon_t, t, mu, logvar)
    print("For bad input, the reconstruction loss is : " + str(BCE_LOSS.item()))

    # 模拟一个还差不多的输入（从样本里选取的）
    data,label = next(iter(train_loader))
    good_input = data[0,0,:,:]
    good_input = good_input.unsqueeze(dim=0).unsqueeze(dim=1)
